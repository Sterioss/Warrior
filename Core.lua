...
-- If we have something we can attack
  if UnitCanAttack('player', target) and player.inmelee and target.infront(target,180) and target.alive
  then
    if player.buff(BattleCry).up and
    (equipeditems >= 4 or itemequipped(TheGreatStormsEye)) and castable(Bladestorm,target) then
    ...
    if (target.debuff(rend).duration <= player.GCD or (target.debuff(rend).duration < 5 and
    player.spell(BattleCry).cooldown < 2 and
    (player.spell(Bladestorm).cooldown < 2 or equipeditems >= 2))) and castable(Rend,target)
    then
      return cast(Rend,target)
    end
    -- If overpower is up and we're not bursting - cast overpower
    if player.buff(Overpower).up and player.buff(BattleCry).down and castable(Overpower,target)
    then
